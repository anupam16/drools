...
public class DynamicClassGenerator {
    ...
    public Object generateDynamicClass(List<ClassName> cnList, Map<String, Object> dynamicObjects) {
        ...
        try {
            String packageName = "com.droolsproject.droolspro.model";
    ...
    private Object getOrCreateClass(String packageName, String className, List<FieldDefinition> fieldDefinitions) {
        try {
            ...
            CtClass dynamicClass3 = pool.get("com.droolsproject.droolspro.model.Device");
            try {
            ...
            try {
            	toStringMethodBody = generateToStringMethod("Device", fieldDefinitions);
            ...
            if (true) {
                ...
                saveToClassFile("Device", bytecode2);
            }
    ...
    private static void saveToClassFile(String className, byte[] bytecode) {
        try (FileOutputStream fos = new FileOutputStream("target\\classes\\com\\ict\\model\\" + className + ".class")) {
            fos.write(bytecode);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void updateInterfaceMethods(Object dynamicObject, String interfaceClassName) {
        try {
            ...
            try {
            	ClassPool pool = ClassPool.getDefault();
                CtClass ct = pool.get("com.droolsproject.droolspro.model" + "." + interfaceClassName);
